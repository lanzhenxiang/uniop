...
vmware.vcdConsole = function(contentId, toolbarId, statusId) {
    ...
    var wrapper = $('<div id="sobey-vmrc"></div').attr('class', 'vmrcWrapper').height('1').width('1');
    ...
    initializeToolbar = function () {
        ...
        powerButtonManager = vmware.buttonManager({container: span})
            .createButton("power-on", {defaultState: {image: sobey.console.imagesUrl+"/console/vm-poweron.png", text: "Power On".localize()}})
            .registerHandler("power-on", showConfirmation, {
                title: "Confirm Power On".localize(), 
                message: "Power On Virtual Machine?".localize(), 
                handler: function(){}
            })
            .createButton("power-pause", {defaultState: {image: sobey.console.imagesUrl+"/console/vm-suspend.png", text: "Suspend".localize()}})
            .registerHandler("power-pause", showConfirmation, {
                title: "Confirm Suspend".localize(), 
                message: "Suspend Virtual Machine?".localize(), 
                handler: function(){}
            })
            .createButton("power-off", {defaultState: {image: sobey.console.imagesUrl+"/console/vm-poweroff.png", text: "Power Off".localize()}})
            .registerHandler("power-off", showConfirmation, {
                title: "Confirm Power Off".localize(), 
                message: "Power Off Virtual Machine?".localize(), 
                handler: function(){}
            })
            .createButton("power-reset", {defaultState: {image: sobey.console.imagesUrl+"/console/vm-reset.png", text: "Reset".localize()}})
        ...
        $('<img />').attr('src', sobey.console.imagesUrl+'/console/separator.png').attr('alt', '').addClass('separator').appendTo(toolbar);

        span = $('<span></span>').attr('id', 'plugin-buttons');
        span.appendTo(toolbar);
        pluginButtonManager = vmware.buttonManager({container: span})
            .createButton("ctrl-alt-del", {defaultState: {image: sobey.console.imagesUrl+"/console/ctrl-alt-del-disabled.png", text: "Send ctrl+alt+del (disabled)".localize()}, connected: {image: sobey.console.imagesUrl+"/console/ctrl-alt-del.png", text: "Send ctrl+alt+del".localize()}})
            .registerHandler("ctrl-alt-del", function(event) {
                if (event.eventData.state == "connected") {
                    console.sendCtrlAltDelete();
                }
            })
            .createButton("fullscreen", {defaultState: {image: sobey.console.imagesUrl+"/console/full-screen-disabled.png", text: "Open in fullscreen (disabled)".localize()}, connected: {image: sobey.console.imagesUrl+"/console/full-screen.png", text: "Open in fullscreen".localize()}})
    ...
    var that = {
        ...
        init: function(opts) {
            ...
            //opts = opts ? opts : {persistent: false, advancedConfig: 'usebrowserproxy=true;tunnelmks=true'};
            opts = opts ? opts : {};
            ...
            var start = function () {
            	console.startup(opts);
            	console.connect(opts.ticketPieces);
            }
