...
vmware.console = function(parent) {
    ...
    var isMsie = /msie/.test(navigator.userAgent.toLowerCase());//$.browser.msie;
    ...
    function registerInternalViewEvents() {
        ...
        //3全屏  ＝＝
        registerEvent("onFullscreenChange", function(width, height) {
            view.trigger(vmware.events.SIZE_CHANGE, {width: width, height: height});
        });
        //4 onGrabStateChange
        ...
        //5 onMessage==
        registerEvent("onMessage", function(msgType,message) {
            view.trigger(vmware.events.MESSAGE, {msgType: msgType,message:message});
        });
        //6 onDeviceStateChange==
        registerEvent("onDeviceStateChange", function(deviceState,hostname,datacenter,vmID,virtualDeviceKey,physicalClientDeviceKey,userRequested,reason) {
            view.trigger(vmware.events.DEVICE_STATE_CHANGE, {deviceState:deviceState,hostname:hostname,datacenter:datacenter,vmID:vmID,virtualDeviceKey:virtualDeviceKey,physicalClientDeviceKey:physicalClientDeviceKey,userRequested:userRequested,reason:reason});
        });
        //7 onVirtualDevicesChange==
        registerEvent("onVirtualDevicesChange", function() {
            view.trigger(vmware.events.VIRTUAL_DEVICES_CHANGE, {});
        });
        //8 onPhysicalClientDevicesChange ==
        registerEvent("onPhysicalClientDevicesChange", function() {
            view.trigger(vmware.events.PHYSICAL_CLIENT_DEVICES_CHANGE, {});
        });
    }
    ...
    //console.log(vmware.normalizeConstants(control[0]));
    ...
    that.startup = function(opts) {
        ...
        opts = opts ? opts : {};
        vmware.log("DEBUG", "plugin", "calling startup({0}, {1}, {2})".format(opts.mode || view.VMRC_Mode.VMRC_MKS, opts.messageMode || view.VMRC_MessageMode.VMRC_EVENT_MESSAGES, opts.advancedConfig || ''));
        view.startup(opts.mode || view.VMRC_Mode.VMRC_MKS, opts.messageMode || view.VMRC_MessageMode.VMRC_EVENT_MESSAGES,opts.advancedConfig || '');
        that.showConsole();
    };
    ...
    that.setScreenSize = function(width,height) {
    	vmware.log("TRACE", "console", "setScreenSize called");
        vmware.log("DEBUG", "plugin", "calling setScreenSize({0},{1})".format(width, height));
        return view.setScreenSize(parseInt(width) || 720, parseInt(height) || 400);
    };
     ...
     */
    that.connect = function(ticketPieces) {
    	
        vmware.log("DEBUG", "plugin", "calling connect({0}, {1}, {2}, {3})".format(ticketPieces.host, ticketPieces.thumb.replace(/-/g, ':'),ticketPieces.allowSSLErrors, ticketPieces.vmid));

        return view.connect(ticketPieces.host, ticketPieces.thumb.replace(/-/g, ':'), (ticketPieces.allowSSLErrors == 1)? true:false || false, ticketPieces.ticket || '', ticketPieces.user, ticketPieces.pass, ticketPieces.vmid, ticketPieces.datacenter || '', ticketPieces.vmPath || '');
