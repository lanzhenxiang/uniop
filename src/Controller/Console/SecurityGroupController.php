...
class SecurityGroupController extends ConsoleController
{
    ...
    public function groupList()
    {
        ...
        $security_group_relation_networks_table = TableRegistry::get('SecurityGroupRelationNetworks');
        ...
        $this->_pageList['rows'] = $query->offset($request['offset'])->limit($request['limit'])->map(function ($row) {
            //实例数
            $row['entry_count'] = TableRegistry::get('SecurityGroupRelationNetworks')->find()->where(['securityGroupCode' =>  $row['code']])->count();
            //vpc名
...
//                $this->set('instanceTypeCode',$firewall_instanceTypeCode);
    ...
    public function delGroup()
    {
        ...
        foreach (explode(',', trim($request['ids'], ',')) as $key => $value) {
            ...
            $data_response = $order->postInterface($url, $array);
    ...
    public function addRule()
    {
        ...
        try {
            ...
            if($request['action-type']=='securitygroup'){
                $array['actionObject']=$request['code'];
            }else{
                $array['actionObject']=$request['source_ip'];
            }

            if ($request['protocol'] == 'all' || $request['protocol'] == 'icmp') {
                $array['portRange']='-1';
            }
    ...
    public function securityGroupEntry()
    {
        ...
        // 同一vpc下，其他的安全组
        $security_data = $basic_table
        ->find()
        ->contain(['SecurityGroupExtends'])
        ->where([
            'InstanceBasic.vpc' => $data['vpc'], 
            'InstanceBasic.type' => 'securityGroup', 
            'InstanceBasic.code <>' => '', 
            'InstanceBasic.code <>' => $data['code'],
            'SecurityGroupExtends.fusionType' => $data['security_group_extend']['fusionType']
            ])
        ->toArray();
        $this->set('security_data', $security_data);
    }
...
//        });
    ...
    public function addEntry()
    {
        ...
        $basic_table = TableRegistry::get('InstanceBasic');
        //安全组信息
        $info = $basic_table->find()->contain(['SecurityGroupExtends'])->where(array('InstanceBasic.id' => $request['basic_id']))->first();
        if (empty($info)) {
            $this->render('/Console/SecurityGroup/index');
        }
        ...
        //子网
        $subnet = $basic_table->find()
        ->contain(['SubnetExtend'])
        ->where([
            'InstanceBasic.type' => 'subnet',
            'InstanceBasic.vpc' => $info['vpc'],
            'InstanceBasic.code <>' => '',
            'SubnetExtend.fusionType' => $info['security_group_extend']['fusionType']
            ])
        ->toArray();
    ...
    public function addEntryList()
    {
        ...
        //安全组code
        $group_data = $basic_table->find()->contain(['SecurityGroupExtends'])->where(array('InstanceBasic.id' => $request['basic_id']))->first();
        $group_code = $group_data['code'];
        
        // 同一种虚拟化技术下的subnet
        $subnet_data = $basic_table->find()->contain(['SubnetExtend'])
        ->where([
            'InstanceBasic.type' => 'subnet', 
            'InstanceBasic.code <>' => '',
            'InstanceBasic.vpc' => $group_data['vpc'],
            'SubnetExtend.fusionType' => $group_data['security_group_extend']['fusionType']
        ])->toArray();
        
        $subnet_array = array();
        
        if (!empty($subnet_data)) {
            foreach ($subnet_data as $s) {
                $subnet_array[] = $s['code'];
            }
        }
        ...
        $matching = array();
        //已用网卡
        $network_arr = $network_table->find()->select(['networkCardCode'])->where(array('securityGroupCode' => $group_code, 'networkCardCode <>' => ''))->toArray();
        ...
        if (!empty($network_arr)) {
            ...
            if (!empty($network_code)) {
                if (isset($request['tab']) && $request['tab'] == 'beenused') {
                    ...
                    // 虚拟化技术
                    $where['InstanceBasic.subnet in'] = $subnet_array;
                }
            }
        } else {
            if (isset($request['tab']) && $request['tab'] == 'beenused') {
                 $this->_pageList['total'] = 0;
                 $this->_pageList['rows'] = array();
                 echo json_encode($this->_pageList);
                 exit;
            } else {
                // 虚拟化技术
        ...
        $query = $card_tabe->find()
        ->contain(['InstanceBasic' => function ($q) use ($matching) {
            return $q->where($matching);
        }])
        ->where($where)
        ->matching('InstanceBasic', function ($q) use ($matching) {
            return $q->where($matching);
        });
        $this->_pageList['total'] = $query->count();
        $this->_pageList['rows'] = $query->offset($request['offset'])->limit($request['limit'])->map(function ($row) use ($group_code) {
    ...
    public function createSecurityGroupArray(){
        ...
        foreach ($agentInfo as $index => $item) {
            if ($item['parentid'] == 0) {
                $agent = array();
                $agent['company'] = array('name' => $item['agent_name'], 'companyCode' => $item['agent_code'], 'price' => HostsController::getPrice($item['id'], 'securityGroup', '元/'),);
    ...
    public function getAreaListByIdVT($id, $agentInfo)
    {
        ...
        foreach ($agentInfo as $index => $item) {
            if ($item['parentid'] == $id) {
                ...
                $str[] = array('name' => $item['agent_name'], 'areaCode' => $item['region_code'], 'virtual'=>$virtualList,'vpc'=>$this->getSelectVpc($item['class_code'],$this->getOwnByDepartmentId()));
            }
        }
        return $str;
    }
