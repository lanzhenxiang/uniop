...
class BusinessController extends ConsoleController
{
    private $_popedomName = array(
//        'hosts' => 'ccm_ps_hosts',
        'hosts' => 'ccm_ps_business_system',
        'disks' => 'ccm_ps_disks',
    ...
    protected function _get_datas_snap($id){
        ...
        $agent = TableRegistry::get('Agent');
        $agent_code = $agent->getAgentRootCode($hosts[0]['class_code']);
    ...
    */
    ...
    public function add($subject = 'hosts')
    {
        ...
        try {
            ...
            $popedomname = $this->request->session()->read('Auth.User.popedomname') ? $this->request->session()->read('Auth.User.popedomname') : 0;
            $templates = $business_template->find()->contain('BusinessTemplateDetail')->where(['status'=>'已发布'])->group('BusinessTemplate.biz_tid')->toArray();
