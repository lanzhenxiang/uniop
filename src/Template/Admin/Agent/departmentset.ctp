    ...
    <div class="content-operate clearfix">
        <div class="clearfix">
            <div class="pull-left">
                ...
                <!--<a type="button" id="btnBack" onclick="return delSession('departmentIDList')" href="<?php echo $this->Url->build(array('controller'=>'Agent','action'=>'arealist'));?>/<?= $_agent["id"] ?>" class="btn btn-addition"><i class="icon-arrow-left"></i>&ensp;返回</a>-->
                <a type="button" id="btnBack" onclick="return delSession('departmentIDList')"  class="btn btn-addition"><i class="icon-arrow-left"></i>&ensp;返回</a>
            </div>
            <div class="input-group content-search pull-right">
                <input type="text" class="form-control" id="searchtext" placeholder="搜索租户名、CODE">
...
$(function(){
        ...
        $("#search_d_t").bind("click", function(){
            $.session.remove('departmentIDList');
                refresh()
          });
});
 function deletes(id){
...
$('#save').on('click',function(){
    var str = $.session.get('departmentIDList');
    var name = $('#searchtext').val();
    //获取所有选中的id
    $.ajax({
        type:'post',
        url:'<?php echo $this->Url->build(array('controller' => 'Agent','action'=>'saveDepartmentPower')); ?>',
        data:{req:str,agentid:"<?= $_area["id"] ?>"},
        success:function(data){
            $.session.remove('departmentIDList');
            tentionHide("操作成功", 0);
            location.href = "<?php echo $this->Url->build(array('controller'=>'Agent','action'=>'departmentset'));?>/<?= $_agent["id"] ?>/<?= $_area["id"] ?>/"+name;
        }
    });
});
...
function getCheckIds(){
    ...
    $.session.set('departmentIDList',strn);
}
function checkId(id){
