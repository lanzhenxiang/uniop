        ...
        <div class="section-header clearfix order-manage">
           <!--  <h5 class="pull-left">
            ...
            <div class="pull-right">
                <button style="margin-left: 1px;" class="btn btn-addition" onclick="search()">查询</button>
            </div>
            ...
            <div class="dropdown pull-right">
                ...
                <ul class="dropdown-menu" id="agent_two">
                        <li><a href="<?= $this->Url->build(['prefix' => 'admin', 'controller' => 'orders', 'action' => $this->request->params['action'],$start,$end,'all',$search]); ?>?search=<?=$search?>&create_by=<?=$create_by?>">全部</a></li>
                        <?php
                        foreach($departments as $value) {
                            ?>
                            <li><a href="<?= $this->Url->build(['prefix' => 'admin', 'controller' => 'orders', 'action' => $this->request->params['action'],$start,$end,$value['id'],$search]); ?>?search=<?=$search?>&create_by=<?=$create_by?>" ><?php echo $value['name'] ?></a></li>
                            <?php
                        }?>
                </ul>
            </div>
            ...
            <table class="table table-striped table-hover vertical-table">
                ...
                if (isset($data) && count($data)>0) {
                    ...
                    <tr>
                        ...
                        <td>
                            <a  href="<?= $this->Url->build(['prefix' => 'admin', 'controller' => 'orders', 'action' => 'orderlog',$value['id']])?>" class="btn btn-xs btn-default">订单日志</a><br/><br/>
                            <!-- 审批按钮 -->
...
$("#charge_mode").change(function(){
  ...
  switch($(this).val()){
    ...
    //修改ecs配置
function changeConfigEcs(index){
    var i = new Array();
...
<script type="text/javascript">
    ...
    var myDate = new Date();
        ...
        $('#datetimepicker-end').datetimepicker({
            ...
            // alert(end);
            location.href="<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'Orders', 'action' =>$this->request->params['action'],$start]);?>/"+end+'/<?=$department['id']?>?search=<?php echo $search?>&create_by=<?=$create_by?>';
        });
        $('#datetimepicker-start').datetimepicker({
            ...
            // alert(end);
            location.href="<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'Orders', 'action' =>$this->request->params['action']]);?>/"+start+'/<?php echo $end?>/<?=$department['id']?>?search=<?php echo $search?>&create_by=<?=$create_by?>';
        });
    ...
    function search(){
        ...
        location.href="<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'Orders', 'action' =>$this->request->params['action'],$start,$end]);?>?search="+search+'&create_by='+create_by;
    }
