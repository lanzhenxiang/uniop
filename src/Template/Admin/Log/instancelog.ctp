...
<div class="wrap-nav-right wrap-index-page">
    <div class="order-main section">
        ...
        <!--class="icon-refresh"></i>&nbsp;&nbsp;刷新</a>-->
                <a  href="<?=$this->Url->build(['controller'=>'Log','action'=>'instancelog']);?>"  class="btn btn-addition"><i
                        class="icon-refresh"></i>&nbsp;&nbsp;刷新</a>
            </div>
            ...
            <div class="pull-right order-number">
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu">
                    <li><a href="<?= $this->Url->build(['prefix' => 'admin', 'controller' => 'log', 'action' => 'instancelog',-1,$start,$end,$loginname]);?>" >全部</a></li>
                    <?php
                    if(isset($departments)){
                      foreach($departments as $value) { ?>
                        <li><a href="<?= $this->Url->build(['prefix' => 'admin', 'controller' => 'log', 'action' => 'instancelog',$value['id'],$start,$end,$loginname]);?>" ><?= $value['name'] ?></a></li>
                        <?php }
                    } ?>
                  </ul>
                </div>
...
<script type="text/javascript">
    var myDate = new Date();
        ...
        $('#datetimepicker-end').datetimepicker({
            ...
            // alert(end);
            location.href="<?= $this ->Url ->build(['prefix' =>'admin', 'controller' =>'log', 'action' =>'instancelog',$department['id'],$start]);?>/"+end+"/<?=$loginname?>";
        });
        $('#datetimepicker-start').datetimepicker({
            ...
            // alert(end);
            location.href="<?= $this ->Url ->build(['prefix' =>'admin', 'controller' =>'log', 'action' =>'instancelog',$department['id']]);?>/"+start+"/<?php echo $end;?>/<?=$loginname?>";
        });
    ...
    $('#search').on('click',function(){
        ...
        location.href = "<?php echo $this->Url->build(array('controller'=>'Log','action'=>'instancelog',$department['id'],$start,$end));?>/"+loginname;
    })
    $(function() {
