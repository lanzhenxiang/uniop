...
<div class="wrap-nav-right">
	<div class="wrap-manage">
        <div class="top">
            <span class="title">分配EIP到负载均衡</span><a href="<?= $this->Url->build(['controller'=>'network','action'=>'lists','eip']); ?>" class="btn btn-addition">返回EIP列表</a>
		...
		<div class="bot ">
		    <table id="table" data-toggle="table" data-pagination="true"
                   ...
                   data-locale="zh-CN" data-click-to-select="true" data-url="<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action' => 'network', 'elb', 'bindEipElbList','?'=>['class_code2'=>$_EipId->location_code,'department_id'=>$department_id,'eipCode'=>$_EipId->code]]); ?>" data-pagination="true"   data-unique-id="A_Code">
            <thead>
             <tr>
                <!-- <th data-checkbox="true"></th> -->
                <th data-field="A_ID" >Id</th>
                <th data-field="A_Code" >负载均衡Code</th>
                <th data-field="A_Name">负载均衡名称</th>
                <th data-field="A_Status" data-formatter="formatter_state">状态</th>
                <th data-field="C_Code" data-formatter="formatter_isBindEip">是否已绑定EIP</th>
...
$("#btnBind").on('click',function(){
    ...
    var eip = isUserEip(info.C_Code,$("#eipId").val());
    if(eip){
...
function refreshTable(type,staute) {
    var state = $("#is_bindEip").attr("state");
    $('#table').bootstrapTable('refresh', {
                url: "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'network','elb','bindEipElbList','?'=>['class_code2'=>$_EipId->location_code]]); ?>",
    ...
    $("#txtsearch").on('keyup',
        function() {
            ...
            var timer = setTimeout(function(){
                ...
                $('#table').bootstrapTable('refresh', {
                    url: "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'network','elb','bindEipElbList','?'=>['class_code2'=>$_EipId->location_code]]); ?>",
                    query: {search: search,staute: staute,department_id:$("#txtdeparmetId").val(),eipCode:$("#eipId").val()}
                });
            },1000);
        });
