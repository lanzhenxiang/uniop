            ...
            <div class="pull-right">
                <div class="dropdown">
                    ...
                    <div class="bot ">
                      ...
                      data-locale="zh-CN" data-click-to-select="true" data-url="<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action' => 'network', 'elb', 'bindElbHostsList','?'=>['department_id'=>$_default['id']]]); ?>" data-pagination="true"   data-unique-id="A_Code">
                      <thead>
                       <tr>
    ...
    bindings: {
        'unwrap': function(event) {
            ...
            if(row.B_Code!=null&&row.B_Code!=""){
                showModal('解绑', '是否解绑ELB',row.A_Name);
...
function refreshTable(type) {
    ...
    $('#table').bootstrapTable('refresh', {
        ...
        query: {class_code2:class_code2,class_code: class_code,staute: state,department_id:$("#txtdeparmetId").val()},
        silent: type
    });
}
...
function showModal(title, content, content1) {
    ...
    html += '<div class="modal-footer"><button id="btnMk" type="button" class="btn btn-primary">确定</button><button type="button" class="btn btn-danger" data-dismiss="modal">关闭</button></div></div></div></div>';
    $("#maindiv").append(html);
