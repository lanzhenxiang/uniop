<!-- author by lixin -->
<!-- TODO 添加反向引入列表在此 -->
<?= $this -> element('import/lists/left', ['active_action' => 'router']); ?>
<div class="wrap-nav-right">
	<div class="wrap-manage">
		...
		<table id="table" data-toggle="table"
    	 ...
    	 data-url="<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'Import', 'hosts', 'routerlists','?'=>['department_id'=>$_default["id"]]]); ?>"
    	 data-unique-id="Identifier">
    	 <thead>
    	   <tr>
    		<th data-checkbox="true"></th>
    		<th data-field="code" ="true">路由器Code</th>
    		<th data-field="display_name">部署区位</th>
    		<th data-field="name" >所属VPC</th>
    	  </tr>
    	</thead>
      </table>
		</div>
...
function refreshTable() {
        ...
        $('#table').bootstrapTable('refresh', {
            url :　"<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'Import','hosts','routerlists']); ?>?department_id="+$("#txtdeparmetId").val()
        });
    }
...
function reverseImport(){
	...
	if(info == ''){
		...
		showModal('提示', 'icon-question-sign', '确认要引入路由器', '', 'doReverseImport()');
	}
}
...
function departmentlist(id,name){
	...
	var class_code2 = $("#agent_t").attr('val');
	$("#table").bootstrapTable('refresh', {
	    url: "<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'Import', 'hosts', 'routerlists']); ?>?search=" + search + '&class_code=' + class_code + '&class_code2=' + class_code2+'&department_id='+$("#txtdeparmetId").val()
	});
}
function refreshTable() {
    var search = $("#txtsearch").val();
...
function local(id, class_code, agent_name) {
    if (agent_name) {
    	...
    	var url = "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'Import','hosts','routerlists']); ?>?search=" + search + '&class_code=' + class_code +'&department_id='+$("#txtdeparmetId").val();
...
function local_two(class_code2, agent_name, class_code) {
    ...
    $('#table').bootstrapTable('refresh', {
    	url: "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'Import','hosts','routerlists']); ?>",
    	query: {
    		class_code2: class_code2,
    		class_code: class_code,
    		search: search,
    ...
    var url = "<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'Import', 'hosts', 'routerlists']); ?>?search=" + search + '&class_code=' + class_code + '&class_code2=' + class_code2+'&department_id='+$("#txtdeparmetId").val();
    $('#table').attr('data-url', url);
}
