<!-- author by lixin -->
<!-- TODO 添加反向引入列表在此 -->
<?= $this -> element('import/lists/left', ['active_action' => 'subnet']); ?>
<div class="wrap-nav-right">
	<div class="wrap-manage">
		<div class="top">
			<span class="title">子网</span>
		</div>
		...
		<table id="table" data-toggle="table"
    	 ...
    	 data-url="<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'Import', 'hosts', 'subnetlists','?'=>['department_id'=>$_default["id"]]]); ?>"
    	 data-unique-id="Identifier">
    	 <thead>
    	   <tr>
    		<th data-checkbox="true"></th>
    		<!-- <th data-field="H_ID" ="true">Id</th> -->
    		<th data-field="code" ="true">子网Code</th>
            <th data-field="router_code">所属路由器</th>
    		<th data-field="display_name">部署区位</th>
    		<th data-field="info" data-formatter="formatter_wd">网段</th>
            <th data-field="info" data-formatter="formatter_wg">默认网关</th>
    	  </tr>
    	</thead>
      </table>
		</div>
...
function formatter_wd(value, row, index) {
	if (value != null) {
...
function formatter_wg(value, row, index) {
    if (value != null) {
        var json =  $.parseJSON(value);
        console.log(json);
        return json.Gateway;
...
function reverseImport(){
	...
	if(info == ''){
		...
		showModal('提示', 'icon-question-sign', '确认要引入子网', '', 'doReverseImport()');
	}
}
...
function departmentlist(id,name){
	...
	$("#table").bootstrapTable('refresh', {
	    url: "<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'Import', 'hosts', 'subnetlists']); ?>?search=" + search + '&class_code=' + class_code + '&class_code2=' + class_code2+'&department_id='+$("#txtdeparmetId").val()
	});
}
function refreshTable() {
    ...
    var class_code2 = $("#agent_t").attr('val');
    $('#table').bootstrapTable('refresh', {
        url :　"<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'Import', 'hosts', 'subnetlists']); ?>?department_id="+$("#txtdeparmetId").val(),
        query: {
...
function local(id, class_code, agent_name) {
    if (agent_name) {
    	...
    	var url = "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'Import','hosts','subnetlists']); ?>?search=" + search + '&class_code=' + class_code +'&department_id='+$("#txtdeparmetId").val();
...
function local_two(class_code2, agent_name, class_code) {
    ...
    $('#table').bootstrapTable('refresh', {
    	url: "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'Import','hosts','subnetlists']); ?>",
    	query: {
    		class_code2: class_code2,
    		class_code: class_code,
    		search: search,
    ...
    var url = "<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'Import', 'hosts', 'subnetlists']); ?>?search=" + search + '&class_code=' + class_code + '&class_code2=' + class_code2+'&department_id='+$("#txtdeparmetId").val();
    $('#table').attr('data-url', url);
}
