<!-- author by lixin -->
<!-- TODO 添加反向引入列表在此 -->
<?= $this -> element('import/lists/left', ['active_action' => 'hosts']); ?>
<div class="wrap-nav-right">
	<div class="wrap-manage">
		...
		<table id="table" data-toggle="table"
    	 ...
    	 data-url="<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'Import', 'hosts', 'ecslists','?'=>['department_id'=>$_default["id"]]]); ?>"
    	 ...
    	 <thead>
    	   <tr>
            ...
            <th data-field="type" >主机类型</th>
    		<th data-field="os_family" >操作系统</th>
    		<th data-field="display_name">部署区位</th>
            <th data-field="vpc_code">所属VPC</th>
    		<th data-field="subnet_code">所属子网</th>
    		<th data-field="ipaddress">主机IP</th>
...
function formatter_config(value, row, index) {
	if (row.cpu != 0) {
		return row.cpu + "核*" + row.memory + "GB*" + row.pgu + "MB";
	} else {
		return "-";
	}
}
...
function departmentlist(id,name){
	...
	$("#table").bootstrapTable('refresh', {
	    url: "<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'Import', 'hosts', 'ecslists']); ?>?search=" + search + '&class_code=' + class_code + '&class_code2=' + class_code2+'&department_id='+$("#txtdeparmetId").val()
	});
}
function refreshTable() {
    ...
    var class_code2 = $("#agent_t").attr('val');
    $('#table').bootstrapTable('refresh', {
        url :　"<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'Import', 'hosts', 'ecslists']); ?>?department_id="+$("#txtdeparmetId").val(),
        query: {
...
function local(id, class_code, agent_name) {
    if (agent_name) {
    	...
    	var url = "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'Import','hosts','ecslists']); ?>?search=" + search + '&class_code=' + class_code +'&department_id='+$("#txtdeparmetId").val();
...
function local_two(class_code2, agent_name, class_code) {
    ...
    $('#table').bootstrapTable('refresh', {
    	url: "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'Import','hosts','ecslists']); ?>",
    	query: {
    		class_code2: class_code2,
    		class_code: class_code,
    		search: search,
    ...
    var url = "<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'Import', 'hosts', 'ecslists']); ?>?search=" + search + '&class_code=' + class_code + '&class_code2=' + class_code2+'&department_id='+$("#txtdeparmetId").val();
    $('#table').attr('data-url', url);
}
...
function reverseImport(){
	...
	if(info == ''){
		...
		showModal('提示', 'icon-question-sign', '确认要引入主机', '', 'doReverseImport()');
	}
}
