    ...
    <div class="wrap-manage">
        ...
        <div class="center clearfix">
            <button class="btn btn-addition" onclick="refreshTable();">
             ...
             <!-- 跨租户新建主机 -->
            <?=$this->element('switchDepartment',['callback_url' => $this->Url->build(['controller' => 'securityGroup', 'action' => 'addGroup']),'typeName'=>'安全组'])?>
            <!-- 跨租户新建主机 -->
            <!-- <a class="btn btn-addition" href="<?= $this->Url->build(['controller' => 'SecurityGroup', 'action' => 'addGroup']) ?>">
                <i class="icon-plus"></i>&nbsp;&nbsp;新建安全组
            </a> -->
        ...
        <div class="bot ">
            ...
            <tr>
                ...
                <th data-field="security_group_extend"  data-formatter="formatter_note">备注</th>
            </tr>
            </thead>
            </table>
        </div>
...
//切换租户
    ...
    function notifyCallBack(value){
		if(value.MsgType=="success"||value.MsgType=="error"){
			if(value.Data.method=="security_group_add"||value.Data.method=="security_group_del"){
				refreshTable();
			}
		}
	}
    ...
    function formatter_note(value, row, index) {
        if (value != null){
            if (value.note != null) {
                return value.note;
            }
