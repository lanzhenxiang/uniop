...
<div class="wrap-nav-right">
   <div class="wrap-manage">
		...
		<a class="btn btn-addition" href="<?= $this -> Url -> build(['controller' => 'fics', 'action' => 'add', 'fics']) ?>">
	 ...
	 data-url="<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'fics', 'fics', 'lists','?'=>['department_id'=>$_default['id']]]); ?>"
...
$("#bntSetting").on('click', function() {
	...
	window.location = "/console/fics/lists/settinglist?f="+rows[0].vol_id;
});
$("#bntRelation").on('click', function() {
	var rows = $('#table').bootstrapTable('getSelections');
	window.location = "/console/fics/lists/ficsHosts?vol_id="+rows[0].vol_id;
});
...
function showModal(title, icon, content, content1, method, type, waringInfo = 0) {
	$("#maindiv").empty();
	html = "";
	...
	html += '</div><div class="modal-body"><i class="'+icon+' icon-color-gray text-primary"></i>&nbsp;&nbsp;' + content + '<span class="text-primary">' + content1 + '</span><br />';
	if (waringInfo == 1) {
...
$('#table').contextMenu('context-menu', {
	bindings: {
					...
					$('#sumbiter').one('click', function() {
						...
						$.getJSON("/console/home/getUserLimit", function(data){
							...
							if(ok==1) {
								//ajax提交页面
								$.ajax({
									url: "<?= $this -> Url -> build(['controller' => 'ajax', 'action' => 'fics', 'fics', 'edit']); ?>",
									async: false,
		...
		'del': function(event) {
			...
			var row = $('#table').bootstrapTable('getRowByUniqueId', uniqueId);
			//获取数据
			showModal('提示', 'icon-question-sign', '确认删除所选存储卷吗？', row.vol_name, 'ajaxFun(\'' + row.vol_id + '\')', 1, 1);
		}
	}
...
$("#btnDel").on('click', function() {
    ...
    if (id != "") {
    	showModal('提示', 'icon-question-sign', '确认要删除存储卷', names, 'ajaxFun(\'' + id + '\')', 1, 1);   
    } else {
...
$("#txtsearch").on('keyup', function() {
	...
	var timer = setTimeout(function() {
		...
		$('#table').bootstrapTable('refresh', {
			url: "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'fics','fics','lists']); ?>?search=" + search + '&class_code=' + class_code + '&class_code2=' + class_code2+'&department_id='+$("#txtdeparmetId").val()+'&type='+type
		});
	}, 1000);
});
...
function ajaxFun(id) {
	...
	$.ajax({
		type: "post",
		url: "<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'fics', 'fics', 'deleteId']); ?>",
		async: true,
		data: {
			id:id
		},
...
function refreshTable() {
	...
	var class_code2 = $("#agent_t").attr('val');
	$('#table').bootstrapTable('refresh', {
		url: "<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'fics', 'fics', 'lists']); ?>",
		query: {
...
function local(id, class_code, agent_name) {
	if (agent_name) {
		...
		var url = "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'fics','fics','lists']); ?>?class_code=" + class_code + "&search=" + search+'&department_id='+$("#txtdeparmetId").val();
...
function local_two(class_code2, agent_name, class_code) {
	...
	var url = "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'fics','fics','lists']); ?>?class_code=" + class_code + "&class_code2=" + class_code2 + "&search=" + search+'&department_id='+$("#txtdeparmetId").val();
	$('#table').attr('data-url', url);
}
...
function notifyCallBack(value) {
	...
	var class_code2 = $("#agent_t").attr('val');
	var type = $("#dl_store_type").attr("val");
	var url = "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'fics','fics','lists']); ?>?search=" + search + '&class_code=' + class_code + '&class_code2=' + class_code2+'&department_id='+department_id+'$type='+type;
	//console.log(value);
