   ...
   <div class="wrap-manage">
		...
		<div class="bot ">
			<table id="table" data-toggle="table"
				...
				data-url="<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'fics', 'fics', 'relevanceDeviceList','?'=>['department_id'=>$_default['id'],'vol_id'=>$vol_id]]); ?>"
	  				...
	  				<div id="mount-type" class="modal-form-group" style="display:inline;">
						...
						<div>
						  	<select id="mount-type-select" name="mount-type">
						  		...
						  		<option value="net use">net use(windows)</option>
<!-- 						  		<option value="unc">unc</option> -->
						  		<option value="mount">mount(linux)</option>
						  	</select>
						</div>
	  				</div>
...
function back(){
	window.location = "/console/fics/lists/fics";
}
//动态创建modal
...
function refreshTable() {
	...
	$('#table').bootstrapTable('refresh', {
		url: "<?= $this->Url->build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'fics', 'fics', 'relevanceDeviceList']); ?>?vol_id="+vol_id+"&type="+deviceType+"&plat_form="+osType+"&auth="+auth+"&search="+search+"&department_id="+departmentId,
	});
}
...
function notifyCallBack(value) {
	...
	var url = "<?= $this->Url->build(['prefix'=>'console','controller'=>'ajax','action'=>'fics','fics','lists']); ?>?search=" + search + '&class_code=' + class_code + '&class_code2=' + class_code2+'&department_id='+department_id;
	//console.log(value);
...
function set(){
	...
	$.ajax({
    		type: "post",
    		url: "<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'ajax', 'action' =>'fics', 'fics', 'setFicsRelationHosts']); ?>",
    		async: true,
    		timeout: 9999999,
    		data: {
    			basic_id: id,
