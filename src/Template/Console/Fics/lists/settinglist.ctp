...
<div class="wrap-nav-right wrap-nav-right-left storage-volume">
   <div class="wrap-manage">
		<div class="top">
			...
			<a href="<?= $this->Url->build(['controller'=>'fics','action'=>'lists','fics']); ?>" class="btn btn-addition">返回列表</a>
		...
		<div class="center clearfix">
			...
			<div class="images-checkbox">
                <div class="bot" style="padding:0;">
                    <table id="table_1" data-toggle="table"
                                         ...
                                         data-url="<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'fics', 'fics', 'settinglist','?'=>['department_id'=>$_default["id"],'t'=>'1','vol_name'=>$_template["vol_name"]]]); ?>"
				...
				<div class="bot" style="display:none;padding:0;">
					<table id="table_4" data-toggle="table"
                                         ...
                                         data-locale="zh-CN"
                                         data-click-to-select="true"
                                         data-url="<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'fics', 'fics', 'settinglist','?'=>['department_id'=>$_default["id"],'t'=>'4','vol_name'=>$_template["vol_name"]]]); ?>"
                                         data-unique-id="id" class="table-bordered">
	 					<thead>
						<tr>
							<th data-field="id">id</th>
          ...
          <table id="table_0" data-toggle="table"
                     ...
                     data-locale="zh-CN"
                     data-click-to-select="true"
                     data-url="<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'fics', 'fics', 'settinglist','?'=>['department_id'=>$_default["id"],'t'=>'0','vol_name'=>$_template["vol_name"]]]); ?>"
                     data-unique-id="id" class="table-bordered">
	 <thead>
						<tr>
                            <th data-field="id">id</th>
...
$('#modal-add-form').bootstrapValidator({
    ...
    submitHandler: function(validator, form, submitButton){
        ...
        //ajax提交页面
        $.ajax({
            url: "<?= $this -> Url -> build(['controller' => 'ajax', 'action' => 'fics']); ?>/<?php echo "fics" ?>/<?php echo "addUser" ?>",
            async: true,
        ...
        name: {
            validators: {
                ...
                stringLength: {
                    ...
                    message: '用户名已被使用',
                    url: '<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'fics', 'fics', 'checkVolUser']) ?>',
                    data: {
                        type: 'name'
                    }
                }
...
function delUser(tab,uniqueId){
    ...
    // console.log(row);
    $.ajax({
        url: "<?= $this -> Url -> build(['controller' => 'ajax', 'action' => 'fics']); ?>/<?php echo "fics" ?>/<?php echo "delUser" ?>",
        data: {
            id:row.userid,
...
function departmentlist(id,name){
	...
	$("div[id*='table']").bootstrapTable('refresh', {
		url: "<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'fics', 'fics', 'settinglist']); ?>",
		query: {
			department_id: department_id
		}
	});
	url = "<?= $this -> Url -> build(['prefix' => 'console', 'controller' => 'ajax', 'action' => 'fics', 'fics', 'settinglist']); ?>?department_id=" + department_id;
...
function checkhtml(){
	...
	$.ajax({
		type: "post",
		url: "<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'ajax', 'action' =>'fics', 'fics', 'cheklist']); ?>",
		async: true,
		data: {
			basic_id:basic_id,
...
function saveFics(){
	...
	$.ajax({
		type:"post",
		url: "<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'ajax', 'action' =>'fics', 'fics', 'savefics']); ?>",
		data:{
			id:id,
			account:account,
			account1:account1,
