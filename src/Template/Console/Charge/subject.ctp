<?= $this -> element('charge/left', ['active_action' => 'subject']); ?>
<!-- author by lixin -->
<!-- TODO 添加消费列表在此 -->
...
<div class="wrap-nav-right">
		...
		<div class="top">
		 ...
		 <div class="center clearfix">
			...
			<?php if (in_array('ccm_user_charge_detail', $this->Session->read('Auth.User.popedomname'))) { ?>
				<button class="btn btn-addition" onclick="location.href='<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'Charge', 'action' =>'detail',$department_id,'citrix',$start,$end]);?>'">消费明细</button>
				<?php }?>
			</div>
		</div>
	</div>
					...
					<div class="graph-display-block">
		                <div class="graph-display-content clearfix">
		                    <div class="graph-display-canvas pull-left">
		                        ...
		                        <div class="text-center" style="margin-top:15px;"><?= $start?>到<?= $end?>消费共计: <span class="text-primary">￥<?php if(!empty($sum_cost)){ echo  sprintf("%.4f",$sum_cost);}else{echo '0';}?></span></div>
		                    </div>
		                    <div class="graph-display-info pull-right">
		                        ...
		                        <?php $_i = 0; foreach($query as $_q_v){?>
		                        	<li>
		                                ...
		                                <?php if(!empty($_q_v['name'])){ echo $_q_v['name'];}?>：￥<?php if(!empty($_q_v)){ echo sprintf("%.4f",$_q_v['cost']);}else{ echo 0;}?>
		                            </li>
		                        <?php $_i++; }?>
		                        </ul>
		                    </div>
	...
	<div class="wrap-manage">
		<div class="center bot">
			...
			data-url="<?= $this->Url->build(['prefix'=>'console','controller'=>'Charge','action'=>'subjectData',$department_id,$start,$end]); ?>"
			data-unique-id="id"
			>
			<thead>
				<tr>
...
<script type="text/javascript">
	...
	$(document).ready(
    	function(){
	        ...
	        $('#datetimepicker-end').datetimepicker({
	            ...
	            location.href="<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'Charge', 'action' =>'subject',$department_id,$start]);?>/"+end;
	        });
	        $('#datetimepicker-start').datetimepicker({
	            ...
	            location.href="<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'Charge', 'action' =>'subject',$department_id
	             ]);?>/"+start+'/<?php echo $end?>';
	        });
    	}
    );
    ...
    // 	location.href="<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'charge', 'action' =>'subject']);?>/"+y+'/'+m;
    // } 
	...
	function handle(value,row ,index){
		var param = "";
//		if(value == 'ecs' || value == 'citrix' || value == 'citrix'){
//
//		}
        param = '?charge_type='+row.charge_type;

		url = "<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'Charge', 'action' =>'detail',$department_id]);?>/"+value+"/<?= $start?>/<?= $end?>";
		if(param != ""){
			url += param;
		}
