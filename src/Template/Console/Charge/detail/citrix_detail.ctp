	...
	<div class="wrap-manage">
		...
		<div class="center">
			...
			<div class="center clearfix bot">
				...
				data-url="<?= $this->Url->build(['prefix'=>'console','controller'=>'Charge','action'=>'detail',$department_id,$type,$start,$end]); ?>?name=<?php if(array_key_exists('name',$this->request->query)){echo $this->request->query['name'];}?>&charge_type=<?php if(array_key_exists('charge_type',$this->request->query)){echo $this->request->query['charge_type'];} ?>"
				data-unique-id="id"
				>
				<thead>
					<tr>
...
<script type="text/javascript">
	...
	function citrixSearch(){
		...
		location.href="<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'Charge', 'action' =>'detail',$department_id,$type,$start,$end]);?>?name="+name+"&charge_type="+charge_type;
