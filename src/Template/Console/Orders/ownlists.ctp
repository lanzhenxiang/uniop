        ...
        <div class="section-header clearfix order-manage">
            ...
            <div class="dropdown pull-right">
                ...
                <ul class="dropdown-menu" id="agent_two">
                    <li><a href="<?= $this->Url->build(['prefix' => 'console', 'controller' => 'orders', 'action' => $this->request->params['action'],$start,$end,0]); ?>?search=<?=$search?><?php if(isset($_GET['file_position'])&&$_GET['file_position']=='admin'){echo '&file_position=admin';}?>">全部</a></li>

                    <li><a href="<?= $this->Url->build(['prefix' => 'console', 'controller' => 'orders', 'action' => $this->request->params['action'],$start,$end,-1]); ?>?search=<?=$search?><?php if(isset($_GET['file_position'])&&$_GET['file_position']=='admin'){echo '&file_position=admin';}?>" >未完结</a></li>

                    <li><a href="<?= $this->Url->build(['prefix' => 'console', 'controller' => 'orders', 'action' => $this->request->params['action'],$start,$end,1]); ?>?search=<?=$search?><?php if(isset($_GET['file_position'])&&$_GET['file_position']=='admin'){echo '&file_position=admin';}?>" >已完结</a></li>
                </ul>
            </div>
            ...
            <div class="fixed-table-container">
                ...
                if (isset($data) && count($data)>0) {
                        ...
                        <tr>
                            ...
                            <td><?php  if($value['order_type']== 0){
                            echo __('新购');
                        }else {
                            echo __('修改配置');
                        }?>
                            </td>
                            <td><?php  if($value['is_console']== 0){
                            echo __('商城购物');
                        }else {
                            echo __('资源中心');
                        }?>
                            ...
                            <?php endif;?>
                            <td><?=$value['orders_goods'][0]->chargeTxt?></td>
                            <td><?=  $this->Number->format($value['orders_goods'][0]->price_per, ['places' => 4,'before' => '¥ '])?>元/<?=$value['orders_goods'][0]->UnitText ?></td>
                            <td><?= $this->Number->format($value['orders_goods'][0]->transaction_price, ['places' => 4,'before' => '¥ '])?>元/<?=$value['orders_goods'][0]->UnitText ?></td>
                            <td>
                        ...
                        <?php }}else{?>
                        <tr>
                            <td colspan="14" style="text-align: center;">没有匹配记录</td>
                        </tr>
                        <?php }?>
                        </tbody>
                    </table>
...
<script type="text/javascript">
    ...
    endDate:time
        ...
        // alert(end);
        location.href="<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'Orders', 'action' =>$this->request->params['action'],$start]);?>/"+end+'/<?=$process_status?>?search=<?php echo $search?>'+"<?php if(isset($_GET['file_position'])&&$_GET['file_position']=='admin'){echo '&file_position=admin';}?>";
    });
    ...
    <?php if(isset($end) && $end > 0){?>
        ...
        // alert(end);
        location.href="<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'Orders', 'action' =>$this->request->params['action']]);?>/"+start+'/<?php echo $end?>/<?=$process_status?>?search=<?php echo $search?>'+"<?php if(isset($_GET['file_position'])&&$_GET['file_position']=='admin'){echo '&file_position=admin';}?>";
    });
    ...
    function search(){
        ...
        location.href="<?= $this ->Url ->build(['prefix' =>'console', 'controller' =>'Orders', 'action' =>$this->request->params['action'],$start,$end,$process_status]);?>?search="+search+"<?php if(isset($_GET['file_position'])&&$_GET['file_position']=='admin'){echo '&file_position=admin';}?>";
    }
